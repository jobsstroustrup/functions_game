"""
Functions Game: Enemy Design and Behavior

Author: Zhenyu He (GitHub: jobsstroustrup)
Role: Designed all 9 enemy class hierarchies and their distinct behavior patterns.

Enemy types (9 subclasses inheriting from base Enemy(pygame.sprite.Sprite)):
- TutEne1: tutorial enemy 1 (yellow, stationary target)
- TutEne2: tutorial enemy 2 (circular orbital motion)
- OrdinEne: ordinary rusher (4-direction straight charge)
- SpikeEne: spike-form variant with specialized collision shape
- WeirdEne: irregular trajectory enemy
- HealEne: healing-on-kill variant (rewards player HP when defeated)
- SplitEne: splitter (spawns multiple smaller enemies on death)
- PhantEne: phantom (visual deception variant)
- PartiEne: particle-form multi-body enemy

Architecture:
- Sprite/Group inheritance pattern with shared HP/animation logic
- Movement strategies span: stationary, circular, charge-and-retreat,
  splitting, and self-healing variants
- 4 Sprite Groups: enemies (master set), ord_enemies, heal_enemies, split_enemies
  (subtype groups enable batch operations such as SplitEne mass-spawn on death)

Each enemy class implements update(), draw(), and damage_received() with
class-specific overrides. Pattern: composition of generic Sprite framework
with custom behavior strategies.
"""
