"""
Functions Game: Domain-Specific Language (DSL) Engine

This module implements the core math-to-function DSL for the Functions Game.
Players compose mathematical functions in real-time via keyboard input,
which are then evaluated to control enemy targeting in the pygame game.

Core responsibilities:
- Token stream management (deque-based bidirectional construction)
- Symbolic simplification rules (automatic cancellation of inverse pairs)
- Cost model for function complexity (non-linear exponential scaling)
- Expression parsing and evaluation with domain-error handling

Author: Zhenyu He (GitHub: jobsstroustrup)
Role: Designed and implemented the complete DSL engine, including:
  - Deque stream architecture (append/prepend via ACTIVE_KEY mapping)
  - Algebraic simplification: cancels (a, s), (s, a), (l, e), (e, l), (d, d), (/, *), (-, -)
  - Non-linear cost function: pow(2+linear, 1.5+A+...)
  - Pretty-print formatting and out-of-domain exception handling

修改历史：
1. 因为全部放在func.py文件下，所以所有'func_'部分省去，stream, expression作为这个文件下的全局变量。
2. 对键盘值的相应放在main.py里，所以对update_stream做了一些修改。
3. 加了consume()计算消耗
"""
import math
