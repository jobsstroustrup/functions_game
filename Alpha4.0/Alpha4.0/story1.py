"""
Functions Game: Tutorial Curriculum and Story Mode

Author: Zhenyu He (GitHub: jobsstroustrup)
Role: Designed the 11-level tutorial curriculum (knowledge sequencing) plus the
story mode scaffolding that introduces players to the math-as-keyboard DSL.

The tutorial is built as a learning progression: each level pairs one PNG
backdrop (tut0.png ... tut10.png) with a fixed list of target points, teaching
the player one new operator (or one new combination) at a time. This mirrors
the structure of an exam-style math curriculum, with difficulty scaling from
single operators to nested compositions.
"""
...
def tutorial (surface, scr, num, cbar):
        """
        11-Level Math Curriculum Tutorial for Functions Game

        Designed by Zhenyu He as a knowledge-sequencing exercise:
        - Levels 0-1: single operations (ln, exp, constants)
        - Levels 2-4: basic composition (sin, arcsin)
        - Levels 5-7: multi-operation combinations (2 sin, ln(exp(x)), etc.)
        - Levels 8-10: complex nested functions

        Each level has a PNG background plus a fixed target-point list that teaches
        students to construct functions passing through those points.

        This tutorial exemplifies gamified educational design and was foundational
        for Zhenyu's later thinking on exam-centric AI coaching.
        """
        global iii, xx
        mid = big[num]
