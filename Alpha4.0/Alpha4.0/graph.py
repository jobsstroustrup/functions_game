"""
Functions Game: Visualization and Coordinate System

Pygame rendering pipeline for function curves, UI bars, and coordinate
transformations (world to screen mapping).

Author: Zhenyu He
Contributions:
- Coordinate system abstraction: lof/tof/xof/yof functions
  (bidirectional mapping between world physics coords and screen pixels)
- Bar class hierarchy (HP/MP/Cost/Percent bars)
- Custom color management and exception handling for edge cases
"""
# graph 的用途：控制并记录 MP、HP、percent、cost、以及所有非 enemy 的绘图
